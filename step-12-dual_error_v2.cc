...
namespace Step12
{
  ...
  namespace AdvectionSolver
  {
   ...
   public:
	   ...
	   virtual void solve_problem(int primal_or_dual)=0;
   ...
   template<int dim>
   void
   Solver<dim>::LinearSystem::solve(Vector<double>& solution, 
                                    const FiniteElement<dim>& fe,
                                    int primal_or_dual) const {
   //here the 3rd argument is used to judge if the current equation is the primal system or dual system.
       if(primal_or_dual == 0){   //0 indicate the equation is primal
       ...
       else  //other numbers indicate that the current equation correspond to dual_system
       {
          //direct solver, too slow and doesn't work for very large matrix
          SparseDirectUMFPACK solver;
          solver.initialize(system_matrix);
          solver.Tvmult(solution, right_hand_side);  
       }
   }
   ...
   template<int dim>
   void
   Solver<dim>::solve_problem(int primal_or_dual){    
       setup_system();
       assemble_system();
       /*
       if(primal_or_dual != 0){
          std::cout<<"the dual_matrix is: "<<std::endl;
          linear_system.right_hand_side.print(std::cout);
       }
       */
       linear_system.solve(solution,fe, primal_or_dual);
   }
   template<int dim>
   unsigned int
...
//the primal solver
  ...
  namespace AdvectionSolver
  {
   ...
   private:     
      virtual void assemble_system();      //pure virtual in Solver class
      typedef MeshWorker::DoFInfo<dim>          DoFInfo;
      typedef MeshWorker::IntegrationInfo<dim>  CellInfo;       
      virtual void integrate_cell_term(DoFInfo& dinfo,CellInfo& info);
      virtual void integrate_boundary_term(DoFInfo& dinfo,CellInfo& info);
   ...
   DualSolver<dim>::assemble_system(){
       ...
       (Solver<dim>::dof_handler.begin_active(),Solver<dim>::dof_handler.end(),
        ...
        assembler);
       //Solver<dim>::linear_system.right_hand_side.print(std::cout);
   }
   ...
   {  
   //assembe the cell_term as in the primal_problem, but with higher order fe.
      const FEValuesBase<dim> &fe_v = info.fe_values();
      FullMatrix<double> &local_matrix = dinfo.matrix(0).matrix;
      const std::vector<double> &JxW = fe_v.get_JxW_values ();
            ...
            for (unsigned int i=0; i<fe_v.dofs_per_cell; ++i)
              for (unsigned int j=0; j<fe_v.dofs_per_cell; ++j)
                local_matrix(i,j) += beta_n *
                                     ...
                                     fe_v.shape_value(i,point) *
                                     JxW[point];
            //note here for the dual problem, we should assemble the rhs on the outflow boundary     
            for (unsigned int i=0; i<fe_v.dofs_per_cell; ++i)
   ...
   template<int dim>
   void 
   WeightedResidual<dim>::solve_problem(int primal_or_dual){
      //
      PrimalSolver<dim>::solve_problem(0);
      DualSolver<dim>::solve_problem(1);
   }
   template<int dim>
   void
      ...
      switch(refinement_criterion)
        {
         ...
         case dual_weighted_error_estimator:
         {
            ...
            std::cout<<"after refine: "<< this->triangulation.n_active_cells()<<std::endl;
            break;
         }
